...
function StatusBadge({ status }: { status: DisplayCourse["status"] }) {
...
interface CourseCardProps {
  ...
  index: number;
  onEdit: (course: DisplayCourse) => void;
  onShare: (course: DisplayCourse) => void;
}
...
export default function CoursesDashboard() {
  ...
  function handleEdit(course: DisplayCourse) {
      ...
      <div className="flex flex-col sm:flex-row sm:items-center gap-3 mb-8">
        <div className="flex-1">
          ...
          <p className="text-sm text-muted-foreground mt-0.5">
            {allDisplayCourses.length} courses total &middot;{" "}
            {allDisplayCourses.filter((c) => c.status === "Published").length}{" "}
            published
          </p>
        </div>
